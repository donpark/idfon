//! Idfond IPC client: 4-byte big-endian length-prefixed JSON frames over a
...
//! Zig app host) and the `idfon` CLI.
...
use idfon_protocol::{Request, Response, MAX_FRAME_BYTES};

pub const DEFAULT_SOCKET: &str = "/tmp/idfon/idfond.sock";
...
/// a JSON `Response` (see the connection loop in `crates/idfond/src/main.rs`).
...
/// - `profile` None/empty -> use the `IDFON_PROFILE` env var (if set)
...
pub fn socket_path_for(profile: Option<&[u8]>) -> String {
    ...
    format!("/tmp/idfon-{profile}/idfond.sock")
}
...
mod tests {
    ...
    fn temp_socket(name: &str) -> (String, UnixListener) {
        let dir = std::env::temp_dir().join(format!("idfon-c-{}-{name}", unique_id()));
    ...
    fn socket_path_table() {
        ...
        env::remove_var("IDFON_PROFILE");
        assert_eq!(socket_path_for(None), "/tmp/idfon/idfond.sock");
        assert_eq!(socket_path_for(Some(b"default")), "/tmp/idfon/idfond.sock");
        assert_eq!(socket_path_for(Some(b"")), "/tmp/idfon/idfond.sock");
        assert_eq!(socket_path_for(Some(b"alice")), "/tmp/idfon-alice/idfond.sock");
        assert_eq!(socket_path_for(Some(b"a-b_9")), "/tmp/idfon-a-b_9/idfond.sock");
        // Invalid characters, invalid UTF-8, and >64 chars fall back
        // (mirroring Zig's truncate-then-validate).
        assert_eq!(socket_path_for(Some(b"../evil")), "/tmp/idfon/idfond.sock");
        assert_eq!(socket_path_for(Some(b"sp ace")), "/tmp/idfon/idfond.sock");
        assert_eq!(socket_path_for(Some(&[0xff, b'x'])), "/tmp/idfon/idfond.sock");
        let long = vec![b'a'; 70];
        assert_eq!(socket_path_for(Some(&long)), format!("/tmp/idfon-{}/idfond.sock", "a".repeat(64)));
        assert_eq!(socket_path_for(Some(&vec![b'a'; 64])), format!("/tmp/idfon-{}/idfond.sock", "a".repeat(64)));

        env::set_var("IDFON_PROFILE", "bob");
        assert_eq!(socket_path_for(None), "/tmp/idfon-bob/idfond.sock");
        // Explicit profile beats the env var.
        assert_eq!(socket_path_for(Some(b"carol")), "/tmp/idfon-carol/idfond.sock");
        env::remove_var("IDFON_PROFILE");
    }
    ...
    fn connect_retry_eventually_succeeds() {
        let dir = std::env::temp_dir().join(format!("idfon-c-{}-late", unique_id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("s");
