//! Thin `nufond` launcher. All daemon logic lives in the shared
//! `libiroh_c_ffi.dylib` (entry: `nufon_daemon_run`, implemented in
//! `native/vendor/iroh-c-ffi/src/daemon.rs` over the `nufon-daemon` crate);
...
extern "C" {
    fn nufon_daemon_run(socket_path: *const c_char, data_dir: *const c_char, transport: *const c_char) -> i32;
}
...
fn cstring(value: &str, flag: &str) -> CString {
    CString::new(value).unwrap_or_else(|_| {
        eprintln!("nufond: {flag} must not contain NUL bytes");
        std::process::exit(2);
    })
}
