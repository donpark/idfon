use std::{env, fs, io::{Read, Write}, os::unix::net::UnixStream, path::PathBuf, process::{Child, Command}, thread, time::{Duration, SystemTime, UNIX_EPOCH}};

fn temp_root() -> PathBuf {
    env::temp_dir().join(format!("nufond-process-{}", SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_nanos()))
}
...
fn two_independent_daemons_start_with_distinct_iroh_identities() {
    ...
    let mut first = Command::new(binary)
        .args(["--socket", first_socket.to_str().unwrap(), "--data-dir", first_data.to_str().unwrap(), "--transport", "iroh"])
        .spawn().unwrap();
    let mut second = Command::new(binary)
        .args(["--socket", second_socket.to_str().unwrap(), "--data-dir", second_data.to_str().unwrap(), "--transport", "iroh"])
        .spawn().unwrap();
    ...
    let first_id = first_context["result"]["identity"]["endpoint_id"].as_str().unwrap();
    let second_id = second_context["result"]["identity"]["endpoint_id"].as_str().unwrap();
