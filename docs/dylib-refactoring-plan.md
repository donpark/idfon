...
|---|---|---|
| `Idfon.app/Contents/MacOS/Idfon` | 46MB | 15.2MB |
| `Idfon.app/Contents/MacOS/idfond` | 37MB | 13.2MB |
...
0.103, tokio, opus/cpal): ~40MB of Idfon's 46MB and nearly all of idfond's
...
- **Daemon:** protocol layer, session/resource bookkeeping
  (`idfon-media::MediaService`, `media.session.*` / `media.resource.*`), blob
  storage, agentics.
...
Goal: `idfond.request` framing/paths/retry/error-mapping exist only in Rust;
...
(`idfon-protocol` pulls only serde/serde_json/thiserror — negligible dylib
...
`idfon_client.h` — a separate hand-maintained header, since plain
...
// reads IDFON_PROFILE, "default"/empty fallback, [A-Za-z0-9_-] validation,
// truncate-then-validate at 64 chars).
int32_t idfon_client_socket_path(const char* profile, char* out, size_t cap);
...
// allocates the response; caller frees with idfon_client_result_free.
// Returns 0 on success, negative codes on failure (IDFON_ECONNECT,
// IDFON_EWRITE, IDFON_EREAD, IDFON_ETOOLARGE, IDFON_EINVALID, ...).
// Response JSON is deserialized with serde (idfon_protocol::Response) before
// being handed back, so malformed responses are rejected here, not sniffed;
// `ok` receives Response.ok (1/0) when non-null.
int32_t idfon_client_request(const char* socket_path,
...
- **Daemon launch stays in Zig** (`launchDaemon` fork/exec is natural there,
  awkward in Rust). Contract: if `idfon_client_request` returns
  `IDFON_ECONNECT`, Zig calls `launchDaemon()` and calls request again with a
...
`idfon_client.h`, `idfon-protocol` path dep added to the vendored crate.
...
(`BINARY_RESPONSE_METHODS` in `client.rs`); documented in `idfon_client.h`.
...
**As-built 2026-09-02:** done. `daemonWorker` now calls `idfon_client_request`
(fast attempt → `launchDaemon` on `IDFON_ECONNECT` → 5000ms-window retry);
`daemonError()` maps codes to the historical strings; `profilePaths` gets the
socket path from `idfon_client_socket_path` (data dir still computed in Zig
...
`idfond` (status → ok=1, unknown method → ok=0 + error body).
...
dylib to `zig-out/bin/`. `Idfon` is 46 MB → **7.9 MB**; `otool -L` shows
...
zig-out/bin/Idfon --output Idfon.app` (see `native/README.md`); current
signing is `none` (`signing-plan.txt`).

**As-built 2026-09-02:** the packager does not carry the dylib, so the
`build` script (mirroring the existing `idfond` copy) appends
`cp zig-out/bin/libiroh_c_ffi.dylib 'Idfon.app/Contents/MacOS/'`. With
...
- [ ] Same error strings reach `host.complete` for: daemon not running,
      ...
      invalid.
- [ ] `idfon status` via the socket through the GUI host call path works with
      daemon cold-started (launchDaemon → connect retry path).
- [ ] `IDFON_PROFILE` routing: socket path, data dir, and `idfond` args all
...
- Rust: unit tests in `iroh-c-ffi` for `idfon_client_socket_path` (profile
  ...
  `idfon_client_request` round-trips (happy path, short read, oversized
  response, bad JSON).
...
- `libiroh_c_ffi.a` no longer linked into `Idfon`; dylib in bundle and signed
  plan updated.
...
Target state: `Contents/MacOS/` contains `Idfon` (thin), `idfond` (thin),
...
- Move `idfond`'s core (request handling in `main.rs`, `idfon-media::MediaService`
  wiring, blob store) behind a C-ABI surface: roughly
  `idfon_daemon_start(config)`, `idfon_daemon_stop()`, with the daemon's
  JSON-RPC-over-socket loop living in the dylib.
- `idfond` binary shrinks to argv parsing + `idfon_daemon_start` + signal
  handling.
...
- Payoff: complete dedup, `idfon-media`/`iroh-live` version skew impossible.

**Option B — Rust `dylib` crate-type shared within the workspace (partial
dedup, cheaper).**

- Add `crate-type = ["dylib"]` to a new `idfon-stack` facade crate; `idfond`
...
1. Extract `idfond/src/main.rs` (3,417 lines) into `crates/idfon-daemon-core`
   with a plain Rust API (`Daemon::start(config) -> Handle`); `idfond` main
   becomes a wrapper. No behavior change; ship and verify this alone.
2. Add the C-ABI lifecycle wrappers to `iroh-c-ffi`; idfond links the dylib
   and calls `idfon_daemon_start`. Media bookkeeping (`MediaService`) moves
...
3. Rebuild bundle; both thin binaries + one dylib. Verify the parity
   checklist again, plus: daemon log redirection (`/tmp/idfond-auto-*.log`)
...
- Single copy of the Rust stack in the bundle; `idfond` < 2MB. ✓ (296KB)
- `idfon-media` and `iroh-c-ffi` cannot have divergent `iroh-live` versions. ✓ (one crate graph)
- Both processes share dylib pages in memory (verify with `vmmap`).

### Phase 2 review findings (2026-09-02, fixed before commit)

- **Duplicate media FFI copies** — the most serious find: `idfon-media`
...
- **Install-name flip-flop** — `test-rust.sh` built the vendored dylib
  ...
  path (breaking thin idfond/bundle until the next zig build) and forcing a
...
- Dead code from the daemon move deleted (`argument`, unused default consts
  in idfon-daemon); unused test imports in idfon-client removed; README
  dylib size corrected (38 → 18 MB).

### Phase 2 as-built 2026-09-02

- **Step 0 (client-core extraction):** new workspace crate
  `crates/idfon-client` — pure-Rust IPC client (`Client`, `ClientResponse`,
  ...
  C-ABI tests); `crates/idfon-cli` uses the same core (its framing copy is
  gone).
- **Daemon core:** new workspace crate `crates/idfon-daemon` — the former
  `idfond/src/main.rs` + `blob.rs` with `DaemonConfig`, `run()`,
  `run_blocking()` (own tokio runtime, ctrl_c shutdown). 14 tests moved with
  it.
- **Thin daemon:** `idfond` binary is 296KB: arg parsing + `extern "C"
  idfon_daemon_run(socket, data_dir, transport)`; `build.rs` adds the dylib
  link-search/link-lib and copies the dylib next to the built binary so
  `@executable_path` resolves in `target/<profile>/`.
- **Dylib:** `iroh-c-ffi/src/daemon.rs` exports `idfon_daemon_run` (blocking,
  declared in `idfon_client.h`); dylib grew 16 → 18MB (daemon core moved in).
...
- **Bundle:** 18MB dylib + 7.9MB app + 312KB idfond ≈ **11MB compressed**
  (was ~27MB before Phase 1). Verified: thin idfond serves the CLI, SIGINT
  cleans the socket, `crates/idfond/tests/process.rs` passes, packaged +
...
| ABI misuse (freed buffers, concurrent calls) | Only 3 functions; ownership rule documented at declaration; `idfon_client_result_free` pairs with request |
| `iroh-c-ffi` Cargo.lock churn from `idfon-protocol` path dep | Commit regenerated lock; pin serde versions to match workspace |
