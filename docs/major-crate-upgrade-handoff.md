...
- Recent rename (commit `b58914b`): `idfon-eve-channel` → `eve-idfon-channel` everywhere (crate, binary, npm packages, env vars `EVE_IDFON_CHANNEL_HOLDER` / `EVE_IDFON_CHANNEL_KEY` / `EVE_IDFON_MODEL`). Doc filenames like `docs/idfon-eve-channel.md` were intentionally left.
...
| `ed25519-dalek` | 2 | 3 | `crates/idfon-core/Cargo.toml`, `crates/eve-idfon-channel/Cargo.toml`, `crates/idfon-daemon/Cargo.toml` |
| `rand_core` | 0.6 | 0.10 | `crates/idfon-core/Cargo.toml` (coupled to ed25519-dalek 3) |
| `base64` | 0.22 | 0.23 | `crates/eve-idfon-channel/Cargo.toml` |
...
3. One major at a time, easiest first:
   - `base64 0.22→0.23` (mechanical API tweaks; `crates/eve-idfon-channel`)
   - `hang 0.19→0.20` (`crates/idfon-media`)
   - `jpeg-encoder 0.6→0.7` (vendored)
   - `iroh-mdns-address-lookup 0.4→0.5` (vendored)
   - `ed25519-dalek 2→3` + `rand_core 0.6→0.10` together (`crates/idfon-core`, `crates/eve-idfon-channel`)
...
- Workspace tests passed for all crates except the `idfond` binary test process, which was SIGKILLed by the OS; a targeted retry reproduced the SIGKILL without a Rust failure. Other workspace tests passed, including `idfon-media`, `idfon-daemon`, `idfon-core`, and `eve-idfon-channel`.
