...
/// `LiveActivityBarModel.rows`: today the only producer is the voice-memo
/// upload, and §5 file sends plug in here later.
///
/// Each producer registers a `begin(id:peerId:name:cancel:)` with a way to stop
...
final class TransferCenter {
    ...
    /// Registers an in-flight transaction. `cancel` must stop it; the producer
    /// then calls `finish(id:)`.
