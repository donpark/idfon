...
final class RoomChatViewController: NSViewController, NSTableViewDataSource, NSTableViewDelegate, NSTextFieldDelegate, ChatStoreObserver {
    ...
    override func loadView() {
        ...
        let invite = NSButton(title: "Copy Invite", target: self, action: #selector(copyInviteTapped)); invite.translatesAutoresizingMaskIntoConstraints = false
        ...
        let bar = NSStackView(views: [input, send, invite, leave]); bar.spacing = 8; bar.translatesAutoresizingMaskIntoConstraints = false
    ...
    @objc private func copyInviteTapped() {
        ...
        ChatStore.shared.onBanner?("Room invite copied")
    }
