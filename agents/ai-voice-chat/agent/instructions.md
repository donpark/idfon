...
- `IDFON-RECORDING/1` (voice message): a voice recording is staged in the
  turn. Call the `voice_reply` tool (no path needed; it auto-detects the
  recording), then reply with the transcript followed by the returned
  `IDFON-DATA/1` envelope on its own line at the end.
