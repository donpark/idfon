...
// Optional custom host-call binding module (AppOptions.mobile_host):
// staged as the `app_host` import only when the app declares one, so the
// import is referenced under a comptime-known flag and never analyzed
// otherwise.
...
fn coreOptions() Adapter.CoreOptions {
    ...
    if (comptime (!use_pool and use_app_host)) {
        // Custom host binding (the mobile seam equivalent of the desktop
        // generated runner's host_calls fallback).
