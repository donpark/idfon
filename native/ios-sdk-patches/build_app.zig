...
pub const AppOptions = struct {
    ...
    /// Optional custom host-call binding for mobile targets (iOS/Android):
    /// a module exposing `binding()` (same contract as the desktop
    /// generated runner's host_calls seam). Imported into the staged
    /// mobile wiring as `app_host` and used when no service pool is
    /// staged. Desktop apps keep wiring custom hosts through their own
    /// generated-runner patch.
    mobile_host: ?MobileHostConfig = null,
};

pub const MobileHostConfig = struct {
...
pub const MobileLibOptions = struct {
    ...
    /// Optional custom host-call binding module (see `AppOptions.mobile_host`).
    mobile_host: ?MobileHostConfig = null,
};
...
fn addMobileLibWithTarget(b: *std.Build, dep: *std.Build.Dependency, target: std.Build.ResolvedTarget, optimize: std.builtin.OptimizeMode, options: MobileLibOptions) void {
    ...
    if (options.scene == .canvas) {
        ...
        const mobile_options_mod = mobile_options.createModule();
        exports_mod.addImport("mobile_build_options", mobile_options_mod);
        ...
        // Optional custom host-call binding (see AppOptions.mobile_host):
        // staged as the `app_host` import; the mobile wiring consults the
        // comptime flag and only references the import when it exists.
        if (options.mobile_host) |host_cfg| {
            const host_mod = b.createModule(.{
                .root_source_file = host_cfg.root,
                .target = target,
                .optimize = optimize,
            });
            host_mod.addImport("native_sdk", native_sdk_mod);
            host_mod.link_libc = host_cfg.link_libc;
            for (host_cfg.include_dirs) |dir| host_mod.addIncludePath(dir);
            for (host_cfg.object_files) |obj| host_mod.addObjectFile(obj);
            app_mod.addImport("app_host", host_mod);
        }
        mobile_options.addOption(bool, "custom_host", options.mobile_host != null);
        app_mod.addImport("mobile_build_options", mobile_options_mod);
...
pub fn addAppArtifacts(b: *std.Build, dep: *std.Build.Dependency, app_options: AppOptions) AppArtifacts {
    ...
    if (target.result.os.tag == .ios or target.result.abi.isAndroid()) {
        addMobileLibWithTarget(b, dep, target, optimize, .{
            ...
            .mobile_host = app_options.mobile_host,
        });
    }
