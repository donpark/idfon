...
// Swift sets this via `media_video_set_rotation` on orientation changes.
#[cfg(target_os = "ios")]
static IOS_ROTATION: AtomicU32 = AtomicU32::new(0);

#[cfg(target_os = "ios")]
pub fn set_ios_rotation(deg: u32) {
    IOS_ROTATION.store(deg % 360, Ordering::Relaxed);
}

/// Rotates a compacted NV12 frame (w x h) by `deg` (0/90/180/270). Returns
/// the rotated bytes with swapped dims for 90/270.
#[cfg(target_os = "ios")]
fn rotate_nv12(data: &[u8], w: usize, h: usize, deg: u32) -> (Vec<u8>, usize, usize) {
    let (new_w, new_h) = match deg {
        90 | 270 => (h, w),
        _ => (w, h),
    };
    let mut out = vec![0u8; new_w * new_h * 3 / 2];
    match deg {
        90 => {
            // 90° CW: (x, y) -> (h-1-y, x). Y:
            for sy in 0..h {
                for sx in 0..w {
                    out[sx * new_w + (new_w - 1 - sy)] = data[sy * w + sx];
                }
            }
            // UV: pair grid (w/2 x h/2); dest pair (h/2-1-py, px);
            // dest byte row stride = new_w, pairs are 2 bytes.
            let (pw, ph) = (w / 2, h / 2);
            for py in 0..ph {
                for px in 0..pw {
                    let src = py * w + px * 2;
                    let di = px * new_w + (pw - 1 - py) * 2;
                    out[y_section_offset(w, h) + di..y_section_offset(w, h) + di + 2]
                        .copy_from_slice(&data[y_section_offset(w, h) + src..y_section_offset(w, h) + src + 2]);
                }
            }
        }
        270 => {
            // 90° CCW: (x, y) -> (y, w-1-x). Y:
            for sy in 0..h {
                for sx in 0..w {
                    out[(new_h - 1 - sx) * new_w + sy] = data[sy * w + sx];
                }
            }
            let (pw, ph) = (w / 2, h / 2);
            for py in 0..ph {
        ...
        180 => {
            for sy in 0..h {
                for sx in 0..w {
                    out[(new_h - 1 - sy) * new_w + (new_w - 1 - sx)] = data[sy * w + sx];
                }
            }
            let (pw, ph) = (w / 2, h / 2);
            for py in 0..ph {
                for px in 0..pw {
                    let src = py * w + px * 2;
                    let di = (ph - 1 - py) * new_w + (pw - 1 - px) * 2;
                    out[y_section_offset(w, h) + di..y_section_offset(w, h) + di + 2]
                        .copy_from_slice(&data[y_section_offset(w, h) + src..y_section_offset(w, h) + src + 2]);
                }
            }
...
#[cfg(target_os = "ios")]
fn y_section_offset(w: usize, h: usize) -> usize {
    w * h
}
...
impl AVFoundationCaptureDevice {
    ...
    pub fn new(index: &CameraIndex, req_fmt: RequestedFormat) -> Result<Self, NokhwaError> {
        ...
        // iPhone sensors expose 12MP camera modes; AbsoluteHighestResolution
        // picks one and 4K@30 NV12 is absurd to encode (and slow to copy).
        // Video calls cap at 720p capture — the encoder ladder is 360p, so
        // anything beyond is wasted bandwidth/CPU. Pick the largest supported
        #[cfg(target_os = "ios")]
        if camera_fmt.resolution().width() > 1280 {
            if let Some(best) = formats
                .iter()
                .filter(|f| f.resolution().width() <= 1280)
                .max_by_key(|f| f.resolution().width() as u64 * f.resolution().height() as u64)
                .copied()
            {
                camera_fmt = best;
            }
...
impl CaptureBackendTrait for AVFoundationCaptureDevice {
    ...
    fn frame(&mut self) -> Result<Buffer, NokhwaError> {
        ...
        // Rotate for phone orientation (portrait = 90° CW); the Buffer label
        // must match the rotated dims or the decode shears.
        #[cfg(target_os = "ios")]
        let (bytes, fmt_res) = {
            let deg = IOS_ROTATION.load(Ordering::Relaxed);
            if deg == 0 {
                (bytes, cfmt.resolution())
            } else {
                let (rot, nw, nh) = rotate_nv12(&bytes, cfmt.resolution().width() as usize, cfmt.resolution().height() as usize, deg);
                (rot, nokhwa_core::types::Resolution::new(nw as u32, nh as u32))
            }
        };
        #[cfg(target_os = "ios")]
        let buffer = Buffer::with_timestamp(fmt_res, &bytes, cfmt.format(), capture_ts);
