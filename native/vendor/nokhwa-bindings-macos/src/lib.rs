...
// Vendored idfon fork: silence the wall of cocoa-foundation deprecation
// warnings ("use the objc2 crate instead") — migrating this snapshot to
// objc2 is not worth it; the deprecated APIs work fine on current macOS.
#![allow(deprecated)]
...
mod internal {
    ...
    impl AVCaptureDevice {
        ...
        pub fn set_all(&mut self, descriptor: CameraFormat) -> Result<(), NokhwaError> {
            // macOS device configuration locks are system-exclusive: another
            // process streaming the same camera holds the lock and lock()
            // is rejected. Configuration is optional — capture with the
            // device's current format instead of failing the open (idfon:
            // two app instances on one Mac during a video call).
            if self.lock().is_err() {
                return Ok(());
            }
