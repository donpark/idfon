...
// idfon vendored fork: uninhabited extern statics (`enum Class {}` used as
// the type of `extern static _NSConcreteStackBlock`) become a hard error in
// a future Rust; an inhabited zero-sized type keeps the same ABI.
#[repr(C)]
struct Class { _priv: [u8; 0] }
